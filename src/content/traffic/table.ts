import { TrafficCategorySelection } from "./types";

export const addTableCheckboxes = (doc: Document, categorySelection: TrafficCategorySelection, updateTrafficCategoryCallback: (categorySelection: TrafficCategorySelection) => void) => {
  ...
  const addCheckbox = (rowElem: Element, id: string, checkedByDefault: boolean) => {
    ...
    checkbox.addEventListener('change', (event: Event) => {
      ...
      updateTrafficCategoryCallback(categorySelection);
    });
  ...
  for (const checkbox of Array.from(checkboxes as NodeListOf<HTMLInputElement>)) {
    ...
    if (checkboxIDSplit.length === 1) {
      ...
      checkbox.checked = categorySelection.subcategories.some(
        subcategory => subcategory.category === category && subcategory.subCategory === subCategory);
    }
  }
}
