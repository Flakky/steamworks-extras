...
import { GameWishlistMonthConversions, GameWishlists, GameWishlistConversions } from "../../shared/types/wishlists";
...
const getConversionRates = (labels: string[], wishlists: WishlistsData, wishlistConversions: WishlistConversionsData): number[] => {
    const conversionRates = labels.map((label: string) => {
        ...
        const conversionsForMonth = wishlistConversions.data
            .filter((conversion: GameWishlistConversions) => {
                return new Date(conversion.date) >= monthStart && new Date(conversion.date) < monthEnd;
            })
            .reduce((sum, conversion) => {
                let total = 0;
                Object.values(conversion.monthConversions).forEach((monthConversion: GameWishlistMonthConversions) => {
                    total += monthConversion.totalConversions;
                });
                return sum + total;
            }, 0);
...
const getLabelsForConversionsChart = (wishlistConversions: WishlistConversionsData) => {
    ...
    wishlistConversions.data.forEach(conversion => {
        Object.keys(conversion.monthConversions).forEach((month: string) => {
    ...
    while (currentDate <= endDate) {
        let formattedDate = dateToString(currentDate);
